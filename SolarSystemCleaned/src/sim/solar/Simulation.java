...
class Simulation extends JPanel implements Runnable {
   ...
   int screenSize;         // screen size both x and y
   int frameDelay;          // millisec delay to slow down animation speed 
   int maxSolarCount;        // cannot exceed data rows solarconfig.csv 
   int cyclesPerSolarSystem;   // degrees of rotation allotted to each solar system 
   int pauseDelay;              // millisec delay between solar systems 
   ...
   int screenMid; // mid-screen location 
   ...
   String line;  
   int lineCount = 0; 
   try(BufferedReader br = new BufferedReader(new FileReader("C:\\Users\\home\\Downloads\\SolarSystemCleaned\\classes\\"+fileName)))
   { 
